...
$\tau = \frac{F}{A} = \frac{m*g*cos(\theta)}{A} = \mu\frac{V}{h}$

$a = \mu\frac{V*A}{h*m*cos(\theta)}$
...
What is the time taken for the block to accelerate from rest to 99\pct of its
terminal velocity?
...
\begin{center}
$u_{r} = U_{infty}(1-\frac{R^{2}}{r^2})cos\theta, v_{\theta} = -U_{infty}(1+\frac{R^{2}}{r^2})sin\theta$
\end{center}
...
\subsection*{(b)} Find the time required for a particl to move from the point
...
\subsection*{(b)} Find the velocity at the midpoint $h/2$ for $n$ = 0.5, 1, and
2.
